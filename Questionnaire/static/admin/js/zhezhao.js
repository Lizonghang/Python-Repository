...
function PDF_launch(ob,zzw,zzh,callback,titlestr){
    ...
    var titleHeight=0;
    if(!titlestr){
        PDF_c.innerHTML="<a style='background:url(/images/bt_closed.gif) no-repeat;width:30px;height:30px;margin:-10px -18px 0 0;display:inline;position:relative;float:right;cursor:pointer;' onclick='PDF_close();'></a>";
    }
    else{
        if(titlestr=="no"){
            PDF_c.innerHTML="";
...
function PDF_close(b){
    ...
    if(zheZhaoCallBack){zheZhaoCallBack(b);}
}